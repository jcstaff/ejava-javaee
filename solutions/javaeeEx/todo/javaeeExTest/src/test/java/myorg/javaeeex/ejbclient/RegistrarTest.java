...
public class RegistrarTest extends TestCase {
    ...
    public void testCreatePerson() throws Exception {
        ...
        assertEquals("unexpected addresses", 1, person2.getAddresses().size());
        
        Address address = person.getAddresses().iterator().next();
        Address address2 = person2.getAddresses().iterator().next();
        assertEquals("unexpected address id", 0, address.getId());
        assertFalse("unexpected address id", address2.getId()==0);
    }
    
    public void testChangeAddress() throws Exception {
        log.info("*** testChangeAddress ***");
        
        Person person = registrar.createPerson(makePerson());
        
        Address address2 = new Address(0,"street2", "city2", "state2", "zip2");
        Person person2 = registrar.changeAddress(person, address2);
